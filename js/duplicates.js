		...
		success: function(tracks) {
			...
			if ( pageCounter <= numPages ) {
				...
				for (var t = 0; t < tracks.items.length; t++) {
					...
					tracksCells += `<tr><td>${ tracks.items[t].track.artists[0].name } - ${ tracks.items[t].track.name }</td></tr>`;
				}
				...
				$("#pltracks tbody").append(tracksCells);
				tracksCells = "";
			}
		},
...
function getThisPlaylist(favPlaylistID) {
	$.ajax({
		...
		success: function(thePlaylist) {
			var plName =
			`
			<th><a target="_blank" href="${ thePlaylist.external_urls.spotify }">${ thePlaylist.name }</a></th>
			`;
			$("#pltracks tr.name").append(plName);
		},
